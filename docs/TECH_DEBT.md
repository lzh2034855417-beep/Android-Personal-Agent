...
| FIX-13（TD-11） | 服务商或任意异常正文直接显示、凭据 data class 自动打印密钥 | 非成功响应不读正文，错误类型化映射；StoredApiKey.toString 脱敏，合成泄露回归 |
...
| TD-10 | P2，CloudLlmProvider 同时组 prompt/JSON/HTTP/解析错误；成功响应已限 1 MiB 且严格 UTF-8，HTTP/JSON 仍耦合 | 超长响应读取已限制；仍难以模拟完整协议错误，无法有效取消 | 纯请求构造器+传输接口；两协议成功/空结果/401/429/超时/取消/响应上限测试 |
