...
2026-09-09；P1 为下次公开发布前优先处理，P2 为可靠性/能力债，P3 为维护优化。以下区分代码可证问题与尚未设备复现的风险。没有做外部漏洞扫描，不能由本表推导“无安全问题”。
...
| ID | 优先级 / 证据 | 影响 | 验收条件 |
| --- | --- | --- | --- |
| TD-01 | P1，MainActivity.buildLevelReport Level 0 使用 deviceProfile CPU 频率 | 已读 Root 档案的派生等级可能进入标作普通 API 的报告 | 按来源限制附件和派生数据；读取 Root 后切 Level 0 的 payload 回归不含该数据 |
| TD-02 | P1，backup_rules/data_extraction_rules 仍为模板，只有 allowBackup=false | 部分系统的设备迁移行为未验证；凭据迁移边界不明确 | 显式排除凭据所在 sharedpref 的云备份/设备迁移，Android 26/31+ 恢复测试 |
| TD-03 | P1，无仓库 CI 工作流，仪器测试尚无本轮执行证据 | UI 回归此前只编译未运行，失效选择器长期未被发现 | 每个 PR 跑 JVM/Lint/build；独立模拟器执行 UI 测试，保留报告，不使用发布 Key |
| TD-04 | P1，DeviceProfileSnapshot/页面硬编码“V8 原厂调度” | 未读取调度时仍给出无证据的厂商/调度结论 | 文案只表示 APA 未修改系统；实际 governor 与系统状态按已采数据展示，缺失明确未知 |
...
| TD-09 | P2，AgentChatScreen 展开报告区不独立滚动 | 大字体/小屏/横屏/键盘可能挤压输入框，尚未本轮实测 | 有界预览+滚动，导入长摘要/键盘/字体缩放组合测试 |
...
| TD-13 | P2，Level0ReportBuilder 已包含 Usage Access 排行 | “应用报告未勾选”不等于完全不发送应用使用信息 | 发送前列出具体范围，使用习惯独立选择；验证未选习惯不会出现在 payload |
