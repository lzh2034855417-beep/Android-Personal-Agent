...
4. Scene 文件先做 2 MiB 输入限制和严格 UTF-8 解码；解析及摘要留在本机。请求 API 没有 Scene 参数；历史策略排除本地消息及其他服务商的消息。
5. 同服务商最多保留最近 12 条允许发送的历史消息。先过滤后截取。之前的云端回答仍可能包含之前的数据，发送新消息不等于撤回此前传输；清空对话可阻止后续携带历史。
6. 云端 HTTP 层只向配置端点发送，凭据由本机 Keystore 解密。不测试真实 API、不读取或记录用户 Key。服务商错误文本、响应大小和可取消性仍有技术债。
