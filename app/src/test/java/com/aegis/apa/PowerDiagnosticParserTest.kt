    ...
    fun mapsEstimatedUidPowerToInstalledPackages() {
        val snapshot = PowerDiagnosticParser.parse(
            sections = listOf(
                ...
                RawDiagnosticSection("batterystats", DiagnosticSourceStatus.AVAILABLE, "  Uid u0a123: 245.5 ( cpu=80.0 wifi=20.0 )")
            ),
    ...
    fun sharedUidKeepsEveryPackageAndTruncationStatus() {
        val snapshot = PowerDiagnosticParser.parse(
            sections = listOf(
                ...
                RawDiagnosticSection("batterystats", DiagnosticSourceStatus.TRUNCATED, "Uid u0a123: 50.0", truncated = true)
            ),
